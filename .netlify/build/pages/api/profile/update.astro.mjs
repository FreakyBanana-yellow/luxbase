...
const Schema = z.object({
  ...
  regeln_text: z.string().max(5e3).optional(),
  welcome_text: z.string().max(5e3).optional(),
...
const POST = async ({ request }) => {
  try {
    ...
    const { data: existing, error: exErr } = await supabase.from("creator_config").select("creator_id").eq("creator_id", user.id).maybeSingle();
    if (exErr) throw exErr;
    if (!existing) {
      const { error } = await supabase.from("creator_config").insert({ creator_id: user.id, ...parsed, updated_at: (/* @__PURE__ */ new Date()).toISOString() });
      if (error) throw error;
    } else {
      const { error } = await supabase.from("creator_config").update({ ...parsed, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("creator_id", user.id);
      if (error) throw error;
    }
