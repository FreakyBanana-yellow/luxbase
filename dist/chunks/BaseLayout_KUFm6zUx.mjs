import { c as createComponent, a as renderTemplate, b as renderHead } from './astro/server_CBNIxeik.mjs';
import 'kleur/colors';
import 'html-escaper';
import 'clsx';
...
const $$BaseLayout = createComponent(async ($$result, $$props, $$slots) => {
  return renderTemplate(_a || (_a = __template(['<html lang="de"> <head><title>Registrieren \u2013 Luxbase</title>', `</head> <body class="bg-luxblack text-white flex justify-center items-center min-h-screen px-4"> <form class="bg-black border border-luxgold rounded-xl p-8 w-full max-w-md" id="registerForm"> <h1 class="text-2xl font-bold text-center text-luxgold mb-6">Registrieren</h1> <input id="name" type="text" placeholder="Dein Name" required class="w-full px-4 py-2 mb-4 rounded bg-gray-800 border border-gray-600"> <input id="email" type="email" placeholder="E\u2011Mail" required class="w-full px-4 py-2 mb-4 rounded bg-gray-800 border border-gray-600"> <select id="role" required class="w-full px-4 py-2 mb-4 rounded bg-gray-800 border border-gray-600"> <option value="creator">Creator / Model</option> <option value="agentur">Agentur</option> </select> <div class="relative mb-2"> <input id="password" type="password" placeholder="Passwort" required class="w-full px-4 py-2 pr-10 rounded bg-gray-800 border border-gray-600"> <button type="button" id="togglePw" class="absolute right-2 top-2 text-yellow-400">\u{1F441}</button> </div> <div class="relative mb-2"> <input id="confirmPassword" type="password" placeholder="Passwort wiederholen" required class="w-full px-4 py-2 pr-10 rounded bg-gray-800 border border-gray-600"> <button type="button" id="toggleConfirm" class="absolute right-2 top-2 text-yellow-400">\u{1F441}</button> </div> <ul class="text-sm mb-4" id="passwordCriteria"> <li id="len" class="text-red-400">\u2022 Mind. 8 Zeichen</li> <li id="upper" class="text-red-400">\u2022 Mind. 1 Gro\xDFbuchstabe</li> <li id="lower" class="text-red-400">\u2022 Mind. 1 Kleinbuchstabe</li> <li id="num" class="text-red-400">\u2022 Mind. 1 Zahl</li> <li id="match" class="text-red-400">\u2022 Beide Passw\xF6rter m\xFCssen \xFCbereinstimmen</li> </ul> <button type="submit" class="w-full bg-luxgold text-black font-bold py-2 rounded hover:bg-yellow-400 transition">Registrieren</button> <p id="error" class="text-red-400 text-sm mt-4 text-center"></p> <p id="success" class="text-green-400 text-sm mt-4 text-center"></p> </form> <script type="module">
      ...
      form.addEventListener('submit', async (e) => {
        ...
        if (!checkPassword()) {
          error.textContent = 'Passwort erf\xFCllt nicht alle Anforderungen.';
          return;
        }
        ...
        try {
          const { data, error: regError } = await window.supabase.auth.signUp({
            email,
            password,
            options: {
              data: { name, role }
          ...
          if (regError) {
            ...
            success.textContent = 'Registrierung erfolgreich. Bitte E-Mail best\xE4tigen.';
            form.reset();
    ...
    <\/script> </body> </html>`], ['<html lang="de"> <head><title>Registrieren \u2013 Luxbase</title>', `</head> <body class="bg-luxblack text-white flex justify-center items-center min-h-screen px-4"> <form class="bg-black border border-luxgold rounded-xl p-8 w-full max-w-md" id="registerForm"> <h1 class="text-2xl font-bold text-center text-luxgold mb-6">Registrieren</h1> <input id="name" type="text" placeholder="Dein Name" required class="w-full px-4 py-2 mb-4 rounded bg-gray-800 border border-gray-600"> <input id="email" type="email" placeholder="E\u2011Mail" required class="w-full px-4 py-2 mb-4 rounded bg-gray-800 border border-gray-600"> <select id="role" required class="w-full px-4 py-2 mb-4 rounded bg-gray-800 border border-gray-600"> <option value="creator">Creator / Model</option> <option value="agentur">Agentur</option> </select> <div class="relative mb-2"> <input id="password" type="password" placeholder="Passwort" required class="w-full px-4 py-2 pr-10 rounded bg-gray-800 border border-gray-600"> <button type="button" id="togglePw" class="absolute right-2 top-2 text-yellow-400">\u{1F441}</button> </div> <div class="relative mb-2"> <input id="confirmPassword" type="password" placeholder="Passwort wiederholen" required class="w-full px-4 py-2 pr-10 rounded bg-gray-800 border border-gray-600"> <button type="button" id="toggleConfirm" class="absolute right-2 top-2 text-yellow-400">\u{1F441}</button> </div> <ul class="text-sm mb-4" id="passwordCriteria"> <li id="len" class="text-red-400">\u2022 Mind. 8 Zeichen</li> <li id="upper" class="text-red-400">\u2022 Mind. 1 Gro\xDFbuchstabe</li> <li id="lower" class="text-red-400">\u2022 Mind. 1 Kleinbuchstabe</li> <li id="num" class="text-red-400">\u2022 Mind. 1 Zahl</li> <li id="match" class="text-red-400">\u2022 Beide Passw\xF6rter m\xFCssen \xFCbereinstimmen</li> </ul> <button type="submit" class="w-full bg-luxgold text-black font-bold py-2 rounded hover:bg-yellow-400 transition">Registrieren</button> <p id="error" class="text-red-400 text-sm mt-4 text-center"></p> <p id="success" class="text-green-400 text-sm mt-4 text-center"></p> </form> <script type="module">
      const form = document.getElementById('registerForm');
      const pw = document.getElementById('password');
      const confirm = document.getElementById('confirmPassword');
      const error = document.getElementById('error');
